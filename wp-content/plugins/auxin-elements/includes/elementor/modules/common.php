...
class Common {
    ...
    function __construct(){
        ...
        add_action( "elementor/frontend/before_render"   , array( $this, 'render_attributes' ) );
...
selector .child-element{ margin: 10px; }
    ...
    public function add_pseudo_background_controls( $widget, $section_id, $args ){
        ...
        if( ! in_array( $section_id, array('section_custom_css', 'section_custom_css_pro') ) ){
            return;
        }

        if( in_array( $widget->get_name(), array('section', 'column') ) ){
            return;
        }
